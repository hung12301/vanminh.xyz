		...
		public function thembangusernamepassword () {
			...
			if($_POST) {
				if($_POST['username'] && $_POST['password']) {
					$data = FacebookController::loginWithUsernamePassword($_POST['username'], $_POST['password']);

					if($data['error'] == 1) {
						Session::setFlash('error', $data['error_message']);
					} else {
						// Get Info
					...
					Session::setFlash('error', 'Bạn phải nhập đủ tài khoản và mật khẩu !');
				}
			}
