<?php
	...
	{	
		...
		public static function loginWithUsernamePassword ($username, $password) {
			$json = curl("https://api.facebook.com/method/auth.login?credentials_type=password&password=".$password."&email=".$username."&format=json&generate_session_cookies=1&locale=vi_VN&method=auth.login&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32");
		...
		public static function searchGroupByKeyword($keyword,$access_token) {
			$fields = "search?q=".$keyword."&type=group&limit=1000&access_token=" . $access_token;
			$json = curl(FACEBOOK_API . $fields);
			$data = json_decode($json);
		...
		public function getAllFriendOfUser($facebook_id, $access_token) {
			$fields = $facebook_id . '/friends?limit=5000&access_token=' . $access_token;
		...
		public function deleteFriend ($user_id, $access_token) {
			$fields = 'me/friends/' . $user_id . '?access_token=' . $access_token;
			$json = '{
				method: "delete"
			}';
			$json = curl_delete('https://graph.facebook.com/' . $fields, $json);
			echo '<pre>';
			print_r($json);
			echo '</pre>';
			// return $res = json_decode($json);
		...
		public function autoPostGroup ($scheduleID) {
				...
				if(strtotime($schedule['updated_at']) + $schedule['distance'] <= time()) {
					if(!$this->isLimitRun($schedule['id'],$schedule['type'],$schedule['updated_at'],$schedule['distance'],$schedule['pause'],$schedule['pause_time'])) {
						...
						if($schedule['type'] == 'post-group') {
							$this->autoPostGroup($schedule['id']);
						}
						// Type = join-group
